...
async def send_setus(bot,ctx,setu_path,setus,with_url=False,is_to_delete=False):
...
async def my_delete_msg(bot,self_id,msg_id):
    await bot.delete_msg(self_id=self_id,message_id=msg_id)
    await asyncio.sleep(1)


def add_delay_job(task,id=None,delay_time:int=30,args=[],kwargs={}):
    now = datetime.datetime.now()
    scheduler = Scheduler()
    job = scheduler.add_job(task,'date',id=id,run_date=now+datetime.timedelta(seconds=delay_time),args=args)
    scheduler.start()
    return job
